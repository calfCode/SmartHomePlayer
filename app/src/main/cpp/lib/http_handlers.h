...
http_handler_play(raop_conn_t *conn, http_request_t *request, http_response_t *response,
    ...
    if (ptr!=NULL){
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_handler_play ptr=%s",ptr);
        int prefix_len =  (int) (ptr - playback_location);
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_handler_play prefix_len=%d",prefix_len);
        if (prefix_len>0){
            set_uri_prefix(conn->raop->airplay_video, playback_location, prefix_len);
        }
    }else{
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_handler_play ptr=null");
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_handler_play playback_location=%s",playback_location);
        if(playback_location!=NULL){
