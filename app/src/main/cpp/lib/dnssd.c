...
dnssd_init(const char* name, int name_len, const char* hw_addr, int hw_addr_len, int *error, int require_pw)
{
...
#elif USE_LIBDL
    dnssd->module = dlopen("libjdns_sd.so", RTLD_LAZY);
	if (!dnssd->module) {
		if (error) *error = DNSSD_ERROR_LIBNOTFOUND;
        LOGD("can not find libjdns_sd.so");
		free(dnssd);
		return NULL;
	}
    ...
    if (!dnssd->hw_addr) {
        ...
        free(dnssd);
        if (error) *error = DNSSD_ERROR_OUTOFMEM;
        LOGD("dnssd_init return NULL 2");
        return NULL;
    }

    memcpy(dnssd->hw_addr, hw_addr, hw_addr_len);
    LOGD("dnssd_init return OK");
    return dnssd;
}
